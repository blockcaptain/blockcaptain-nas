...
from ._progress import SIZE_256KiB, progress_instance
...
def write_progress_callback(completed, total, task_id):
    progress_instance.update(task_id, completed=completed, total=total)
...
class IsoFile(contextlib.AbstractContextManager):
    ...
    def write_iso(self, path: Path) -> None:
        with progress_instance:
            task_id = progress_instance.add_task("Write ISO")
            logging.info(f"Start writing ISO to '{path}'.")
            self._iso.write(
                str(path),
                blocksize=SIZE_256KiB,
                progress_cb=write_progress_callback,
                progress_opaque=task_id,
            )
